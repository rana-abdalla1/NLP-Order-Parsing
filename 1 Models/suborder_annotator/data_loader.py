...
def process_bert(data, tokenizer, vocab):
    """
    Process Parquet data for use with a BERT model.

    Args:
        data (list): Data as a list of dictionaries loaded from Parquet.
        tokenizer: BERT tokenizer.
        vocab: Vocabulary object.

    Returns:
        Tuple of processed inputs for training and evaluation.
    """
    bert_inputs = []
    grid_labels = []
    grid_mask2d = []
...
def load_data_bert(config):
    """
    Load data for BERT model training and evaluation from Parquet files.

    Args:
        config: Configuration object containing dataset paths and model settings.

    Returns:
        Tuple containing train, dev, and test datasets and raw data.
    """
    import pandas as pd
    ...
    # File paths for train, dev, and test sets
    train_file = f'./data/project/{config.dataset}/train.parquet'
    dev_file = f'./data/project/{config.dataset}/dev.parquet'
    test_file = f'./data/project/{config.dataset}/test.parquet'

    # Load datasets
    train_data = load_parquet(train_file)
    dev_data = load_parquet(dev_file)
    test_data = load_parquet(test_file)
    ...
    # Update config with label and vocabulary information
